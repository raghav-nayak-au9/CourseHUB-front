...
export function listComment(course_no=-1,page=0,limit=0,reload=false) {
    return function (dispatch) {
...
export function removeComment(course_no=-1,page=0,limit=0,comment_no) {
    return function (dispatch) {
        dispatch(commentLoading(true));
        axios.post(`${hostUrl}/remove-comment`, {course_no,comment_no}, {
            headers: {
                'Accept': 'application/json, text/plain, */*',
                'Content-Type': 'application/json',
                'Authorization': localStorage.getItem('token')
...
export function toggleHelpful(course_no=-1,page=0,limit=0,comment_no) {
    return function (dispatch) {
        dispatch(commentLoading(true));
        axios.post(`${hostUrl}/toggle-helpful`, {course_no,comment_no}, {
            headers: {
                'Accept': 'application/json, text/plain, */*',
                'Content-Type': 'application/json',
                'Authorization': localStorage.getItem('token')
